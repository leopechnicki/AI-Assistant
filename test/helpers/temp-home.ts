...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("HEX_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.HEX_STATE_DIR = path.join(base, ".hex");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "hex-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".hex", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
