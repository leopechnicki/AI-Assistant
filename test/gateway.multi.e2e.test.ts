...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `hex-e2e-${name}-`));
  const configDir = path.join(homeDir, ".hex");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          HEX_SKIP_CHANNELS: "1",
          HEX_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        HEX_GATEWAY_TOKEN: inst.gatewayToken,
        HEX_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          HEX_GATEWAY_PORT: String(gwA.port),
          HEX_GATEWAY_TOKEN: gwA.gatewayToken,
          HEX_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          HEX_GATEWAY_PORT: String(gwB.port),
          HEX_GATEWAY_TOKEN: gwB.gatewayToken,
          HEX_GATEWAY_PASSWORD: "",
        }),
