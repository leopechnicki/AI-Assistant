import type { BrowserConfig, BrowserProfileConfig, HexConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_HEX_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "hex" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_HEX_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_HEX_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Hex browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for hex/hex CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: HexConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_HEX_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "hex";
