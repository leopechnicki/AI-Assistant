...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".hex-profile-decorated");
}
 ...
 */
export function decorateHexProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_HEX_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_HEX_BROWSER_COLOR).toUpperCase();
