...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "hex-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".hex", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<HexConfig> = {},
): HexConfig {
  const base: HexConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "hex"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".hex", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
