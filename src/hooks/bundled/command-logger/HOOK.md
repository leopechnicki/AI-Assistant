...
homepage: https://docs.hex.ai/hooks#command-logger
metadata:
  {
    "hex":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.hex/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.hex/logs/commands.log ~/.hex/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/hex`:
   ```
   /home/username/.hex/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
