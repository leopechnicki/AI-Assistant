...
const saveSessionToMemory: HookHandler = async (event) => {
  ...
  try {
    ...
    const cfg = context.cfg as HexConfig | undefined;
    const agentId = resolveAgentIdFromSessionKey(event.sessionKey);
    const workspaceDir = cfg
      ? resolveAgentWorkspaceDir(cfg, agentId)
      : path.join(os.homedir(), ".hex", "workspace");
    ...
    if (sessionFile) {
      ...
      if (sessionContent && cfg) {
        ...
        const hexRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
        const slugGenPath = path.join(hexRoot, "llm-slug-generator.js");
