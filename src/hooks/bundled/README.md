...
This directory contains hooks that ship with Hex. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.hex.ai/hooks/soul-evil
...
Hooks can be configured in `~/.hex/hex.json`:
...
2. Restart gateway: `pkill -9 -f 'hex.*gateway' && pnpm hex gateway`
3. Enable the hook: `hex hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.hex.ai/hooks
