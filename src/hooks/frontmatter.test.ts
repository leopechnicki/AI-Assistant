...
`;
    ...
    expect(parsed.hex.emoji).toBe("💾");
    expect(parsed.hex.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.hex.emoji).toBe("📝");
    expect(parsed.hex.events).toEqual(["command"]);
    expect(parsed.hex.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.hex.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"hex": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"hex": {"events": ["test"]}}');
  });
...
describe("resolveHexMetadata", () => {
  it("extracts hex metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveHexMetadata(frontmatter);
  ...
  it("returns undefined when hex key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        hex: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Hex" },
            { id: "npm", kind: "npm", package: "@hex/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@hex/hook");
  });
    ...
    "hex":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Hex" }],
      },
  }
---
...
`;
    ...
    const hex = resolveHexMetadata(frontmatter);
    expect(hex).toBeDefined();
    expect(hex?.emoji).toBe("💾");
    expect(hex?.events).toEqual(["command:new"]);
    expect(hex?.requires?.config).toEqual(["workspace.dir"]);
    expect(hex?.install?.[0].kind).toBe("bundled");
  });
