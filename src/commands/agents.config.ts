...
export function listAgentEntries(cfg: HexConfig): AgentEntry[] {
...
function resolveAgentName(cfg: HexConfig, agentId: string) {
...
function resolveAgentModel(cfg: HexConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: HexConfig): AgentSummary[] {
