...
beforeAll(() => {
  previousProfile = process.env.HEX_PROFILE;
  process.env.HEX_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.HEX_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/hex/pnpm-lock.yaml",
      markerPath: "/tmp/hex/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Hex status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("hex status --all") ||
          l.includes("hex --profile isolated status --all") ||
          l.includes("hex status --all") ||
          l.includes("hex --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.HEX_GATEWAY_TOKEN = "abcd1234";
    try {
