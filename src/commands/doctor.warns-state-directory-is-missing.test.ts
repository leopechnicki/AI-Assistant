...
beforeEach(() => {
  ...
  resolveHexPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.HEX_STATE_DIR;
  originalUpdateInProgress = process.env.HEX_UPDATE_IN_PROGRESS;
  process.env.HEX_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "hex-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.HEX_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.HEX_UPDATE_IN_PROGRESS;
  } else {
    process.env.HEX_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveHexPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadHexPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "hex-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.HEX_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when HEX_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/hex.json",
      exists: true,
      raw: "{}",
    ...
    process.env.HEX_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
