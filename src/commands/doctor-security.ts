...
import type { HexConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: HexConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("hex security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("hex configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("hex config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("hex doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("hex config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
