...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "HEX_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("HEX_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv HEX_GATEWAY_TOKEN");
    expect(message).not.toContain("HEX_GATEWAY_PASSWORD");
  });
