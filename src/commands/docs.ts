...
const SEARCH_TOOL = "https://docs.hex.ai/mcp.SearchHex";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.hex.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('hex docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.hex.ai/");
      runtime.log(`Search: ${formatCliCommand('hex docs "your query"')}`);
    }
    return;
  }
