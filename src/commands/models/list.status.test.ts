...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveHexAgentDir: vi.fn().mockReturnValue("/tmp/hex-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/hex-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/hex-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveHexAgentDir: mocks.resolveHexAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveHexAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/hex-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/hex-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/hex-agent-custom");
