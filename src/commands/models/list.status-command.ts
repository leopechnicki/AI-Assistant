...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveHexAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("hex models auth setup-token")}\` or \`${formatCliCommand("hex configure")}\`.`
          : `Run \`${formatCliCommand("hex configure")}\` or set an API key env var.`;
