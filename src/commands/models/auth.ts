...
import { readConfigFileSnapshot, type HexConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: HexConfig, model: string): HexConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("hex plugins install")}\`.`,
    );
  }
