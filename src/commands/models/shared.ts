...
export async function updateConfig(
  mutator: (cfg: HexConfig) => HexConfig,
): Promise<HexConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: HexConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: HexConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("hex agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
