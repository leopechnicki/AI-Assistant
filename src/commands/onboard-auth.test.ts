...
const requireAgentDir = () => {
  const agentDir = process.env.HEX_AGENT_DIR;
  if (!agentDir) {
    throw new Error("HEX_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.HEX_STATE_DIR;
  const previousAgentDir = process.env.HEX_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.HEX_AGENT_DIR;
    } else {
      process.env.HEX_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.HEX_OAUTH_DIR;
  });

  it("writes auth-profiles.json under HEX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-oauth-"));
    process.env.HEX_STATE_DIR = tempStateDir;
    process.env.HEX_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.HEX_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to HEX_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-minimax-"));
    process.env.HEX_STATE_DIR = tempStateDir;
    process.env.HEX_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.HEX_AGENT_DIR;
