...
describe("onboard (non-interactive): Cloudflare AI Gateway", () => {
  it("stores the API key and configures the default model", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "hex-onboard-cf-gateway-"));
  ...
  it("infers auth choice from API key flags", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "hex-onboard-cf-gateway-infer-"));
    process.env.HOME = tempHome;
    process.env.HEX_STATE_DIR = tempHome;
    process.env.HEX_CONFIG_PATH = path.join(tempHome, "hex.json");
    vi.resetModules();
