...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.HEX_GATEWAY_PORT).toBe("18789");
    expect(env.HEX_GATEWAY_TOKEN).toBe("secret");
    expect(env.HEX_SERVICE_MARKER).toBe("hex");
    expect(env.HEX_SERVICE_KIND).toBe("gateway");
    expect(typeof env.HEX_SERVICE_VERSION).toBe("string");
    expect(env.HEX_SYSTEMD_UNIT).toBe("hex-gateway.service");
    if (process.platform === "darwin") {
      expect(env.HEX_LAUNCHD_LABEL).toBe("ai.hex.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", HEX_PROFILE: "work" },
      port: 18789,
    });
    expect(env.HEX_SYSTEMD_UNIT).toBe("hex-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.HEX_LAUNCHD_LABEL).toBe("ai.hex.work");
    }
  });
});
