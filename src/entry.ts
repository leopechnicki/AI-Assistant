...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.HEX_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.HEX_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.HEX_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[hex] ${parsed.error}`);
    process.exit(2);
  }
