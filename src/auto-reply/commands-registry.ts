...
export function isCommandEnabled(cfg: HexConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: HexConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as HexConfig),
...
export function getCommandDetection(_cfg?: HexConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: HexConfig) {
