...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-commands-"));
...
function buildParams(commandBody: string, cfg: HexConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
