...
import { scheduleGatewaySigusr1Restart, triggerHexRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Hex in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerHexRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Hex via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
