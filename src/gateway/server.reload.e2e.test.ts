...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.HEX_SKIP_CHANNELS;
    prevSkipGmail = process.env.HEX_SKIP_GMAIL_WATCHER;
    process.env.HEX_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.HEX_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.HEX_SKIP_GMAIL_WATCHER;
    } else {
      process.env.HEX_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
