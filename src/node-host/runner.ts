...
import { ensureHexCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.HEX_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.HEX_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureHexCliOnPath({ pathEnv: process.env.PATH ?? "" });
