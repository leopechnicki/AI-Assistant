...
function setSlackDmPolicy(cfg: HexConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Hex";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Hex`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: HexConfig, allowFrom: string[]): HexConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
