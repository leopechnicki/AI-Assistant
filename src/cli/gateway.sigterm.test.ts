...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "hex-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      HEX_SKIP_CHANNELS: "1",
      HEX_SKIP_GMAIL_WATCHER: "1",
      HEX_SKIP_CRON: "1",
      HEX_SKIP_BROWSER_CONTROL_SERVER: "1",
      HEX_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "hex-entry-bootstrap.mjs");
