...
${formatHelpExamples([
  ['hex agent --to +15555550123 --message "status update"', "Start a new session."],
  ['hex agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'hex agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'hex agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['hex agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'hex agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.hex.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.hex.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['hex agents set-identity --agent main --name "Hex" --emoji "🦞"', "Set name + emoji."],
  ["hex agents set-identity --agent main --avatar avatars/hex.png", "Set avatar path."],
  [
    "hex agents set-identity --workspace ~/.hex/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "hex agents set-identity --identity-file ~/.hex/workspace/IDENTITY.md --agent main",
