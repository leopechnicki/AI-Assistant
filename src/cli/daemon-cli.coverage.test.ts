...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    HEX_STATE_DIR: process.env.HEX_STATE_DIR,
    HEX_CONFIG_PATH: process.env.HEX_CONFIG_PATH,
    HEX_GATEWAY_PORT: process.env.HEX_GATEWAY_PORT,
    HEX_PROFILE: process.env.HEX_PROFILE,
  };

  beforeEach(() => {
    process.env.HEX_STATE_DIR = "/tmp/hex-cli-state";
    process.env.HEX_CONFIG_PATH = "/tmp/hex-cli-state/hex.json";
    delete process.env.HEX_GATEWAY_PORT;
    delete process.env.HEX_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.HEX_STATE_DIR !== undefined) {
      process.env.HEX_STATE_DIR = originalEnv.HEX_STATE_DIR;
    } else {
      delete process.env.HEX_STATE_DIR;
    }

    if (originalEnv.HEX_CONFIG_PATH !== undefined) {
      process.env.HEX_CONFIG_PATH = originalEnv.HEX_CONFIG_PATH;
    } else {
      delete process.env.HEX_CONFIG_PATH;
    }

    if (originalEnv.HEX_GATEWAY_PORT !== undefined) {
      process.env.HEX_GATEWAY_PORT = originalEnv.HEX_GATEWAY_PORT;
    } else {
      delete process.env.HEX_GATEWAY_PORT;
    }

    if (originalEnv.HEX_PROFILE !== undefined) {
      process.env.HEX_PROFILE = originalEnv.HEX_PROFILE;
    } else {
      delete process.env.HEX_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        HEX_PROFILE: "dev",
        HEX_STATE_DIR: "/tmp/hex-daemon-state",
        HEX_CONFIG_PATH: "/tmp/hex-daemon-state/hex.json",
        HEX_GATEWAY_PORT: "19001",
      },
