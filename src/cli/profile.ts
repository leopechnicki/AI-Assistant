...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.hex${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.HEX_PROFILE = profile;

  const stateDir = env.HEX_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.HEX_STATE_DIR?.trim()) {
    env.HEX_STATE_DIR = stateDir;
  }

  if (!env.HEX_CONFIG_PATH?.trim()) {
    env.HEX_CONFIG_PATH = path.join(stateDir, "hex.json");
  }

  if (profile === "dev" && !env.HEX_GATEWAY_PORT?.trim()) {
    env.HEX_GATEWAY_PORT = "19001";
  }
}
