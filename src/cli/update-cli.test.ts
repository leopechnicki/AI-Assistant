...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(resolveHexPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-update-"));
    try {
      await fs.writeFile(
      ...
      vi.mocked(resolveHexPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-update-wizard-"));
    const previousGitDir = process.env.HEX_GIT_DIR;
    try {
      setTty(true);
      process.env.HEX_GIT_DIR = tempDir;
      ...
      process.env.HEX_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
