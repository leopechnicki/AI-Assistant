...
const SANDBOX_EXAMPLES = {
  main: [
    ["hex sandbox list", "List all sandbox containers."],
    ["hex sandbox list --browser", "List only browser containers."],
    ["hex sandbox recreate --all", "Recreate all containers."],
    ["hex sandbox recreate --session main", "Recreate a specific session."],
    ["hex sandbox recreate --agent mybot", "Recreate agent containers."],
    ["hex sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["hex sandbox list", "List all sandbox containers."],
    ["hex sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["hex sandbox recreate --all", "Recreate all containers."],
    ["hex sandbox recreate --session main", "Recreate a specific session."],
    ["hex sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["hex sandbox recreate --browser --all", "Recreate only browser containers."],
    ["hex sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["hex sandbox explain", "Show effective sandbox config."],
    ["hex sandbox explain --session agent:main:main", "Explain a specific session."],
    ["hex sandbox explain --agent work", "Explain an agent sandbox."],
    ["hex sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.hex.ai/cli/sandbox")}\n`,
    )
    .action(() => {
