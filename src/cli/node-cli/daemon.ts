...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("hex node install"), formatCliCommand("hex node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("hex node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    HEX_LOG_PREFIX: baseEnv.HEX_LOG_PREFIX ?? "node",
