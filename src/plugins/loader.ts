...
import { discoverHexPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: HexPluginDefinition;
  register?: HexPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as HexPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as HexPluginDefinition;
...
export function loadHexPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverHexPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "hex/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: HexPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as HexPluginModule;
    } catch (err) {
