...
import { loadHexPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.HEX_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `hex-plugin-${randomUUID()}`);
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.HEX_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadHexPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.HEX_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadHexPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadHexPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
