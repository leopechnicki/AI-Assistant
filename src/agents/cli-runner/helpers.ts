...
/**
 * Cleanup suspended Hex CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: HexConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-cli-images-"));
  const paths: string[] = [];
