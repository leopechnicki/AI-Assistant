...
const defaultTools = createHexCodingTools();

describe("createHexCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createHexCodingTools();
    ...
    try {
      ...
      const contents = "Hello from hex read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "hex-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "hex-workspace"),
    ...
    const tools = createHexCodingTools({ sandbox });
