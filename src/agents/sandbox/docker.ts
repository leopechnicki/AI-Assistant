...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "hex.sandbox=1");
  args.push("--label", `hex.sessionKey=${params.scopeKey}`);
  args.push("--label", `hex.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `hex.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("hex.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`hex sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`hex sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("hex sandbox recreate --all");
}
