...
type WebSearchConfig = NonNullable<HexConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: HexConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("hex configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.hex.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://hex.ai",
      "X-Title": "Hex Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
