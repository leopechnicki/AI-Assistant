...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "hex-models-" });
}

const MODELS_CONFIG: HexConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureHexModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureHexModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureHexModelsJson } = await import("./models-config.js");
      const { resolveHexAgentDir } = await import("./agent-paths.js");

      await ensureHexModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveHexAgentDir(), "models.json");
