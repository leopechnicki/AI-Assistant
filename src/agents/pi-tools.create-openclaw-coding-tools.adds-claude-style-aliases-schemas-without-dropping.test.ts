...
import { __testing, createHexCodingTools } from "./pi-tools.js";
...
describe("createHexCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createHexCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "hex-outside.txt");
