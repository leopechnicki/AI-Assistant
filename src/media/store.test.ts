...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "HEX_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "hex-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.HEX_STATE_DIR = path.join(home, ".hex");
    ...
    await fs.mkdir(path.join(home, ".hex"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.hex${path.sep}media`);
