...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hex-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "hex doctor",
        managerScriptArgs(manager, "hex", ["doctor", "--non-interactive"]),
        gitRoot,
        { HEX_UPDATE_IN_PROGRESS: "1" },
      ),
    );
