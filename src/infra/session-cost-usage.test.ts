...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "hex-cost-"));
    ...
    const originalState = process.env.HEX_STATE_DIR;
    process.env.HEX_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) {
        ...
        process.env.HEX_STATE_DIR = originalState;
      }
    }
  });

  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "hex-cost-session-"));
