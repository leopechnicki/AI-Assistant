...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.HEX_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*hex-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "hex";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "hex");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "hex-gw");
  ...
  records.push(`_hex-gw._tcp IN PTR ${instanceLabel}._hex-gw._tcp`);
  records.push(`${instanceLabel}._hex-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._hex-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; hex-content-hash: ${contentHash}\n${contentBody}`;
}
