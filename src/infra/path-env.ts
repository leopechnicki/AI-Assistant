...
function candidateBinDirs(opts: EnsureHexPathOpts): string[] {
  ...
  // Bundled macOS app: `hex` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "hex");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/hex` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "hex"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `hex` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureHexCliOnPath(opts: EnsureHexPathOpts = {}) {
  if (isTruthyEnvValue(process.env.HEX_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.HEX_PATH_BOOTSTRAPPED = "1";
