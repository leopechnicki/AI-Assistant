...
export function triggerHexRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.HEX_SYSTEMD_UNIT,
        process.env.HEX_PROFILE,
      );
  ...
  const label =
    process.env.HEX_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.HEX_PROFILE);
