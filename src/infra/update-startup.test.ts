...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    const { resolveHexPackageRoot } = await import("./openclaw-root.js");
    ...
    vi.mocked(resolveHexPackageRoot).mockResolvedValue("/opt/hex");
