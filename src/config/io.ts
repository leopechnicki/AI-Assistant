...
import type { HexConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareHexVersions } from "./version.js";
...
function coerceConfig(value: unknown): HexConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as HexConfig;
}
...
function stampConfigVersion(cfg: HexConfig): HexConfig {
...
function warnIfConfigFromFuture(cfg: HexConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareHexVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Hex (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: HexConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): HexConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as HexConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as HexConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: HexConfig) {
    clearConfigCache();
...
// module scope. `HEX_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.HEX_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.HEX_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): HexConfig {
...
export async function writeConfigFile(cfg: HexConfig): Promise<void> {
  clearConfigCache();
