...
import type { HexConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: HexConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: HexConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = HexSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as HexConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as HexConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as HexConfig)),
    ),
  };
}
