...
export function parseHexVersion(raw: string | null | undefined): HexVersion | null {
  if (!raw) {
    return null;
  }
...
export function compareHexVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseHexVersion(a);
  const parsedB = parseHexVersion(b);
  if (!parsedA || !parsedB) {
    return null;
  }
