  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.hex/logs/hex.log" },
        hooks: {
          path: "~/.hex/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".hex", "logs", "hex.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".hex", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".hex", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".hex", "agents", "main"));
