...
const ETC_HEX_DIR = path.join(ROOT_DIR, "etc", "hex");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "hex.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcHexPath(...parts: string[]) {
  return path.join(ETC_HEX_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcHexPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "hex.json"))).toEqual({
      shared: true,
    });
  });
});
