...
# Checks both Claude Code and Hex auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
HEX_AUTH="$HOME/.hex/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    hex models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    hex_status=$(check_hex_auth 2>/dev/null || true)

    claude_expires=0
    hex_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        hex_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        hex_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$HEX_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            hex: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$hex_status" == EXPIRED* ]] || [[ "$hex_status" == MISSING* ]]; then
        echo "HEX_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$hex_status" == EXPIRING* ]]; then
        echo "HEX_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Hex Auth (~/.hex/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'hex doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active hex >/dev/null 2>&1; then
    echo -e "Hex service: ${GREEN}running${NC}"
else
    echo -e "Hex service: ${RED}NOT running${NC}"
fi
