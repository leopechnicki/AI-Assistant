...
ZIP=${1:?"Usage: $0 Hex-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/hex/hex/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Hex-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/hex/hex/releases/download/v${VERSION}/"}
