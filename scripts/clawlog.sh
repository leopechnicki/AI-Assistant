...
show_usage() {
    cat << EOF
clawlog - Hex Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Hex logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Hex logs flow through the macOS unified log (subsystem: ai.hex).
