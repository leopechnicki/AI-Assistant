#!/bin/bash
# Setup Hex Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'hex-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Hex message: Send warning via Hex itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/hex-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/hex-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/hex-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now hex-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status hex-auth-monitor.timer"
echo ""
echo "Quick commands:"
