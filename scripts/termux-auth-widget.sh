...
SERVER="${HEX_SERVER:-${CLAWDBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking Hex auth..."

STATUS=$(ssh "$SERVER" '$HOME/hex/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/hex/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|HEX_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/hex/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|HEX_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Hex won't work until re-auth"
                ;;
        esac
        ;;
