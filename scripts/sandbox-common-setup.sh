...
BASE_IMAGE="${BASE_IMAGE:-hex-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-hex-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=hex.sandbox=1)
NOTE
