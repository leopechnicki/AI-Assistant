...
INSTALL_URL="${HEX_INSTALL_URL:-https://hex.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${HEX_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${HEX_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="hex"
PACKAGE_NAME="${HEX_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (HEX_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${HEX_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${HEX_INSTALL_LATEST_OUT:-}"
fi
