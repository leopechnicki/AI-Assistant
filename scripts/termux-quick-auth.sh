...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/hex/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Hex Re-Auth" -c "After getting key, run: ssh $SERVER '~/hex/scripts/mobile-reauth.sh'" --id hex-auth
        ;;
    *)
