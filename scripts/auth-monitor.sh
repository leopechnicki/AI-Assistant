...
#
# Suggested cron: */30 * * * * /home/admin/hex/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Hex notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., hex-alerts)
...
STATE_FILE="$HOME/.hex/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Hex if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use hex
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Hex to $NOTIFY_PHONE..."
            hex send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Hex is down. Run: ssh l36 '~/hex/scripts/mobile-reauth.sh'" "urgent"
    exit 1
