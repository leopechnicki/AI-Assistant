...
IMAGE_NAME="hex-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/hex-bin:$PATH"
  mkdir -p /tmp/hex-bin

  cat > /tmp/hex-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/hex-bin/systemctl

  cat > /tmp/hex-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  npm_bin="/tmp/npm-prefix/bin/hex"
  npm_entry="/tmp/npm-prefix/lib/node_modules/hex/dist/index.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/hex-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/hex-gateway.service"
    if [ ! -f "$unit_path" ]; then
