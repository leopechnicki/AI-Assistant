...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/hex-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/hex-onboard-${case_name}.log"
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/.hex/workspace"
    config_path="$HOME/.hex/hex.json"
    sessions_dir="$HOME/.hex/agents/main/sessions"
