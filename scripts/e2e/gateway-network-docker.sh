...
IMAGE_NAME="hex-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="hex-net-e2e-$$"
GW_NAME="hex-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "HEX_GATEWAY_TOKEN=$TOKEN" \
	  -e "HEX_SKIP_CHANNELS=1" \
	  -e "HEX_SKIP_GMAIL_WATCHER=1" \
	  -e "HEX_SKIP_CRON=1" \
	  -e "HEX_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
