...
```bash
hex tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Check the Gateway logs: `hex logs --follow`.
- Confirm the agent can run: `hex status` and `hex models status`.
...
- No agents in picker: check `hex agents list` and your routing config.
