...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Hex
...
Running Hex via Node with `tsx` fails at startup with:

```
[hex] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `hex status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch hex.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node hex.mjs status` works on Node 25.
