---
summary: "Quick troubleshooting guide for common Hex failures"
read_when:
...
When Hex misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `hex status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `hex status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `hex status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `hex gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `hex channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `hex gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `hex logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `hex status --all` (it redacts tokens). If you paste `hex status`, consider setting `HEX_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  hex models auth setup-token --provider anthropic
  ```
...
```bash
hex models auth paste-token --provider anthropic
hex models status
```
...
- Preferred: `hex logs --follow`
- File logs (always): `/tmp/hex/hex-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$HEX_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u hex-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  hex config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  hex config set gateway.mode remote
  hex config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `hex setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.hex/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `hex doctor`
...
- then run `hex sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `hex gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `hex gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `hex gateway status` and `hex doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `HEX_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `hex gateway status` shows a config mismatch**
...
- Fix: rerun `hex gateway install --force` from the same `--profile` / `HEX_STATE_DIR` you want the service to use.

**If `hex gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `hex doctor` to update it (or `hex gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `HEX_GATEWAY_TOKEN`) and restart the service.

**If `hex gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/hex` on disk.
...
Symptoms: `pwd` or file tools show `~/.hex/sandboxes/...` even though you
...
Hex intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `hex models list` or
  `hex models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${HEX_CONFIG_PATH:-$HOME/.hex/hex.json}"
```
...
tail -f "$(ls -t /tmp/hex/hex-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
hex logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.hex/agents/<agentId>/sessions/
```
...
hex logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
hex channels logout
trash "${HEX_STATE_DIR:-$HOME/.hex}/credentials" # if logout can't cleanly remove everything
hex channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/hex/hex-*.log | head -1)" | tail -20
```

### High Memory Usage

Hex keeps conversation history in memory.
...
Hex now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `hex doctor` reports every invalid entry.
- `hex doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `hex logs`, `hex health`, `hex status`, `hex gateway status`, and `hex gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/hex/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `hex models status` (CLI).
...
```bash
curl -fsSL https://hex.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. Run `hex channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Hex scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/hex/hex/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
hex gateway status
hex gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.hex.* still works)
```
...
Ensure the global `hex` CLI is installed and matches the app version:

```bash
hex --version
npm install -g hex@<version>
```
...
#   ${HEX_CONFIG_PATH:-$HOME/.hex/hex.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
hex gateway --verbose
hex channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/hex/hex-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$HEX_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.hex/logs/...`; profiles use `~/.hex-<profile>/logs/...`)<br />Linux: `journalctl --user -u hex-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Hex Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$HEX_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$HEX_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$HEX_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
trash "${HEX_STATE_DIR:-$HOME/.hex}"
hex channels login         # re-pair WhatsApp
hex gateway restart           # or: hex gateway
```
...
1. Check logs first: `/tmp/hex/` (default: `hex-YYYY-MM-DD.log`, or your configured `logging.file`)
