...
- During heartbeat runs, Hex treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `hex sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Hex skips the heartbeat run entirely (no model call).
...
```bash
hex system event --text "Check for urgent follow-ups" --mode now
```
