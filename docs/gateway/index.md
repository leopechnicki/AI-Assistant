...
- Replaces the legacy `gateway` command. CLI entry point: `hex gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
hex gateway --port 18789
# for full debug/trace logs in stdio:
hex gateway --port 18789 --verbose
...
- Config hot reload watches `~/.hex/hex.json` (or `HEX_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__hex__/canvas/` from `~/.hex/workspace/canvas`. Disable with `canvasHost.enabled=false` or `HEX_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `HEX_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `HEX_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.hex.*` may still exist)
- Linux: `hex-gateway-<profile>.service`
- Windows: `Hex Gateway (<profile>)`

Install metadata is embedded in the service config:

- `HEX_SERVICE_MARKER=hex`
- `HEX_SERVICE_KIND=gateway`
- `HEX_SERVICE_VERSION=<version>`
...
```bash
hex --dev setup
hex --dev gateway --allow-unconfigured
...
- `HEX_STATE_DIR=~/.hex-dev`
- `HEX_CONFIG_PATH=~/.hex-dev/hex.json`
- `HEX_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.hex/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `HEX_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `HEX_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `HEX_CONFIG_PATH`
- unique `HEX_STATE_DIR`
...
```bash
hex --profile main gateway install
hex --profile rescue gateway install
```

Example:

```bash
HEX_CONFIG_PATH=~/.hex/a.json HEX_STATE_DIR=~/.hex-a hex gateway --port 19001
HEX_CONFIG_PATH=~/.hex/b.json HEX_STATE_DIR=~/.hex-b hex gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `hex health --json`).
...
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `hex gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.hex.*` is cleaned up).
  - `hex doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Hex profile services.
  ...
  - Cleanup: `hex gateway uninstall` (current service) and `hex doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `hex gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Hex.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.hex.*` labels still unload cleanly).
- To stop it cleanly, use `hex gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `hex gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `hex gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Hex installs a **systemd user service** by default on Linux/WSL2. We
...
`hex gateway install` writes the user unit. `hex doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/hex-gateway[-<profile>].service`:

```
[Unit]
Description=Hex Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/hex gateway --port 18789
Restart=always
RestartSec=5
Environment=HEX_GATEWAY_TOKEN=
...
```
systemctl --user enable --now hex-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/hex-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now hex-gateway[-<profile>].service
```
...
- `hex gateway health|status` — request health/status over the Gateway WS.
- `hex message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `hex agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `hex gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `hex gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `hex gateway` and the legacy TCP control port.
