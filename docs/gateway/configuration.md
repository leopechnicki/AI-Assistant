---
summary: "All configuration options for ~/.hex/hex.json with examples"
read_when:
...
Hex reads an optional **JSON5** config from `~/.hex/hex.json` (comments + trailing commas allowed).

If the file is missing, Hex uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.hex/workspace`). You usually only need a config to:
...
Hex only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `hex doctor`, `hex logs`, `hex health`, `hex status`, `hex service`, `hex help`).
- Run `hex doctor` to see the exact issues.
- Run `hex doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `hex config set`. Keep a backup of `~/.hex/hex.json`.
...
```bash
hex gateway call config.get --params '{}' # capture payload.hash
hex gateway call config.apply --params '{
...
```bash
hex gateway call config.get --params '{}' # capture payload.hash
hex gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.hex/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@hex", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.hex/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.hex/workspace" }],
}
```
...
{ "$include": "/etc/hex/base.json5" } // absolute
...
```json5
// ~/.hex/clients/mueller/agents.json5
[
...
```json5
// ~/.hex/clients/mueller/broadcast.json5
{
...
Hex reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Hex runs your login shell and imports only the missing expected keys (never overrides).
...
- `HEX_LOAD_SHELL_ENV=1`
- `HEX_SHELL_ENV_TIMEOUT_MS=15000`
...
Hex stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.hex/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.hex/credentials/oauth.json` (or `$HEX_STATE_DIR/credentials/oauth.json`)
...
- `~/.hex/agent/*` (migrated by `hex doctor` into `~/.hex/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `HEX_OAUTH_DIR`
- Agent dir (default agent root override): `HEX_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Hex imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Hex derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/hex/hex-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/hex/hex.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.hex/credentials/whatsapp/biz
          // authDir: "~/.hex/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `hex doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@hex", "hex"] } }],
  },
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.hex/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.hex/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.hex/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.hex/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.hex/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.hex/workspace-home" },
      { id: "work", workspace: "~/.hex/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `hex.json`).
...
Hex starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["hex-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
Hex starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `hex plugins install @hex/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Hex starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Hex spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Hex
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/hex" } },
}
```
...
When a file exceeds this limit, Hex logs a warning and injects a truncated
...
message envelopes). If unset, Hex uses the host timezone at runtime.
...
`[hex]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[hex]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[hex]"`, Hex will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Hex generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.hex/settings/tts.json",
      elevenlabs: {
...
Hex also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Hex currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `hex configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:hex`: all built-in Hex tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.hex/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "hex-sandbox-browser:bookworm-slim",
          containerPrefix: "hex-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Hex uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.hex/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Hex config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Hex writes/merges a `models.json` into
`~/.hex/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Hex uses
...
- Shortcut: `hex onboard --auth-choice opencode-zen`.
...
Shortcut: `hex onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `hex onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `hex onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `hex onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `hex onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `hex onboard --auth-choice minimax-api`.
...
- Override the agent config root with `HEX_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.hex/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.hex/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.hex/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Hex stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.hex/skills` (workspace skills
...
from `~/.hex/extensions`, `<workspace>/.hex/extensions`, plus any
...
### `browser` (hex-managed browser)

Hex can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for hex and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Hex.app menubar, or `hex gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      hex: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/hex" }
...
- When a connection comes from one of these IPs, Hex uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `hex gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `HEX_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `HEX_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Hex
...
- Hex.app watches `~/.hex/hex.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.hex/hex.json` (or `HEX_CONFIG_PATH`) and applies changes automatically.
...
- `~/.hex/hex.json` (or `HEX_CONFIG_PATH`)
...
- `HEX_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `hex --dev …` → uses `~/.hex-dev` + shifts ports from base `19001`
- `hex --profile <name> …` → uses `~/.hex-<name>` (port via config/env/flags)
...
```bash
HEX_CONFIG_PATH=~/.hex/a.json \
HEX_STATE_DIR=~/.hex-a \
hex gateway --port 19001
```
...
Gmail helper config (used by `hex webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "hex@gmail.com",
...
- Set `HEX_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Hex defaults `serve.path` to `/` so
...
Default root: `~/.hex/workspace/canvas`  
Default port: `18793` (chosen to avoid the hex browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__hex__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__hex__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.hex/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.hex/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.hex/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_hex-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `hex` (advertises `hex.local`). Override with `HEX_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_hex-gw._tcp` under `~/.hex/dns/` using the configured discovery domain (example: `hex.internal.`).
