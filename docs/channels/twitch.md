...
read_when:
  - Setting up Twitch chat integration for Hex
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Hex connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
hex plugins install @hex/twitch
```

Local checkout (when running from a git repo):

```bash
hex plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `HEX_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "hex", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use HEX_TWITCH_ACCESS_TOKEN env var)
...
```bash
HEX_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
