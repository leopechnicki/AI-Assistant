...
```bash
hex plugins install @hex/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
hex plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Hex" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `hex pairing list nextcloud-talk`
  - `hex pairing approve nextcloud-talk <CODE>`
