...
3. Set tokens for Hex and start the gateway.
...
7. Slash Commands → create `/hex` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
Hex can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Hex falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Hex",
    "description": "Slack connector for Hex"
  },
  "features": {
    "bot_user": {
      "display_name": "Hex",
    ...
    "slash_commands": [
      {
        "command": "/hex",
        "description": "Send a message to Hex",
        "should_escape": false
      }
    ]
  },
...
By default, Hex replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Hex infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `hex pairing approve slack <code>`.
...
- On startup, Hex resolves channel/user names in allowlists to IDs (when tokens allow)
