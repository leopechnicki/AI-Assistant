...
Matrix is an open, decentralized messaging protocol. Hex connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `hex plugins install @hex/matrix`
   - From a local checkout: `hex plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Hex calls the same
     login endpoint, stores the access token in `~/.hex/credentials/matrix/credentials.json`,
...
- On first connection, Hex requests device verification from your other sessions.
...
`~/.hex/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, Hex resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
