...
read_when:
  - You want Hex to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Hex to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`hex onboard`) and `hex channels add` list optional channel plugins.
...
```bash
hex plugins install @hex/nostr
```

Use a local checkout (dev workflows):

```bash
hex plugins install --link <path-to-hex>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "hex@example.com",
        "lud16": "hex@example.com"
      }
    }
  }
}
