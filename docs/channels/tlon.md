...
Tlon is a decentralized messenger built on Urbit. Hex connects to your Urbit ship and can
...
```bash
hex plugins install @hex/tlon
```

Local checkout (when running from a git repo):

```bash
hex plugins install ./extensions/tlon
```
...
Use these with `hex message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Hex replies in-thread.
