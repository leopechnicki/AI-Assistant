...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Hex to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
If you just installed Hex, run the wizard:
...
2. Configuring app credentials in Hex
...
1. You already ran `hex channels add` for Feishu
2. The gateway is running (`hex gateway status`)
...
```bash
hex pairing approve feishu <CODE>
```
...
2. Run `hex logs --follow` and look for `chat_id`
...
2. Run `hex logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `hex gateway status`  | Show gateway status           |
| `hex gateway install` | Install/start gateway service |
| `hex gateway stop`    | Stop gateway service          |
| `hex gateway restart` | Restart gateway service       |
| `hex logs --follow`   | Tail gateway logs             |
...
5. Ensure the gateway is running: `hex gateway status`
6. Check logs: `hex logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.hex/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.hex/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
