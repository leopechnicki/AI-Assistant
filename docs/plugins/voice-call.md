...
read_when:
  - You want to place an outbound voice call from Hex
...
Voice calls for Hex via a plugin. Supports outbound notifications and
...
- Use `hex voicecall ...` or the `voice_call` tool
...
```bash
hex plugins install ./extensions/voice-call
