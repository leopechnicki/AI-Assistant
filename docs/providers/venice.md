---
summary: "Use Venice AI privacy-focused models in Hex"
read_when:
  - You want privacy-focused inference in Hex
...
```bash
hex onboard --auth-choice venice-api-key
```
...
```bash
hex chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Hex shows all available Venice models. Pick based on your needs:
...
```bash
hex models set venice/claude-opus-45
hex models set venice/llama-3.3-70b
```

List all available models:

```bash
hex models list | grep venice
```

## Configure via `hex configure`
...
Hex automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
hex chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
hex chat --model venice/claude-opus-45

# Use uncensored model
hex chat --model venice/venice-uncensored

# Use vision model with image
hex chat --model venice/qwen3-vl-235b-a22b

# Use coding model
hex chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `hex models list` to see currently available models. Some models may be temporarily offline.
