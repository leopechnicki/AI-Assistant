---
summary: "Use MiniMax M2.1 in Hex"
read_when:
  - You want MiniMax models in Hex
...
```bash
hex plugins enable minimax-portal-auth  # skip if already loaded.
hex gateway restart  # restart if gateway is already running
hex onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/hex/hex/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `hex.json`:
...
- Use `hex models list` and `hex models set minimax/MiniMax-M2.1` to switch.
...
- Running `hex configure` and selecting **MiniMax M2.1**, or
