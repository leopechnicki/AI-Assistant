---
summary: "Use Synthetic's Anthropic-compatible API in Hex"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Hex registers it as the
...
Note: Hex's Anthropic client appends `/v1` to the base URL, so use
