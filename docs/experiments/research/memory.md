...
read_when:
  - Designing workspace memory (~/.hex/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Hex integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.hex/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.hex/workspace` as canonical human-readable memory.
...
```
~/.hex/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Hex**, but keep a separable core library.

### Why integrate into Hex?
...
- You want the agent itself to call the tools:
  - `hex memory recall "…" --k 25 --since 30d`
  - `hex memory reflect --since 7d`
...
Pragmatic take for `~/.hex/workspace`:
