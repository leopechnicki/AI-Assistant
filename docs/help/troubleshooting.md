...
You can also set `HEX_VERBOSE=1` instead of the flag.
...
### `docs.hex.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.hex.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.hex.ai` to the allowlist, then retry.
...
- Check the allowlist: `hex config get agents.defaults.models`
...
If you can, include the relevant log tail from `hex logs --follow`.
