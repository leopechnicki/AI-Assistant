...
- **gateway host** → `hex` process on the gateway machine
...
`~/.hex/exec-approvals.json` directly.

CLI: `hex approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
