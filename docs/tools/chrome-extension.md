---
summary: "Chrome extension: let Hex drive your existing Chrome tab"
read_when:
...
The Hex Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate hex-managed Chrome profile.
...
Hex then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Hex release (npm package) as static files. There is no separate “build” step.

After upgrading Hex:

- Re-run `hex browser extension install` to refresh the installed files under your Hex state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Hex ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Hex to control.
...
- `ON`: attached; Hex can drive that tab.
...
`hex browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `hex browser extension install` first to copy the extension to a stable location under your Hex state directory.
...
- **This is not isolated** like the dedicated hex-managed profile.
