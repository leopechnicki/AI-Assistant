---
summary: "Agent tool surface for Hex (browser, canvas, nodes, message, cron) replacing legacy `hex-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `hex-*` skills
title: "Tools"
---

# Tools (Hex)

Hex exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `hex-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `hex.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Hex logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`hex node run`).
...
- Requires a Brave API key (recommended: `hex configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Hex-managed browser.
...
- Quick smoke: `hex nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `hex gateway` restart in-place)
