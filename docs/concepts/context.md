...
read_when:
  - You want to understand what “context” means in Hex
...
“Context” is **everything Hex sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Hex-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Hex builds the system prompt

The system prompt is **Hex-owned** and rebuilt each run. It includes:
...
By default, Hex injects a fixed set of workspace files (if present):
