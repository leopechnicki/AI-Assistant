...
- `/usage cost` in chats: local cost summary aggregated from Hex session logs.
- CLI: `hex status --usage` prints a full per-provider breakdown.
- CLI: `hex channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
