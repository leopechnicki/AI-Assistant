...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `hex sandbox explain`.
...
Legacy `agent.*` configs are migrated by `hex doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
4. **Monitor logs:**
   ```exec
   tail -f "${HEX_STATE_DIR:-$HOME/.hex}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
