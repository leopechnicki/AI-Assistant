...
read_when:
  - "You installed Hex but `hex` is “command not found”"
...
Hex’s runtime baseline is **Node 22+**.

If you can run `npm install -g hex@latest` but later see `hex: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
