...
read_when:
  - You need detailed behavior for hex onboard
...
This page is the full reference for `hex onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.hex/hex.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `hex doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `hex pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `hex health`.
    - `hex status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.hex/.env` so launchd can read it.
...
- OAuth credentials: `~/.hex/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.hex/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`hex onboard`](/cli/onboard)
