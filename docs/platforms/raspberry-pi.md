---
summary: "Hex on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Hex on a Raspberry Pi
  - Running Hex on ARM devices
...
Run a persistent, always-on Hex Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Hex features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled hex

# Enable if not
sudo systemctl enable hex

# Start on boot
sudo systemctl start hex
```
...
```bash
# Check logs
journalctl -u hex --no-pager -n 100

# Common fix: rebuild
cd ~/hex  # if using hackable install
npm run build
sudo systemctl restart hex
```
...
- [Termux (Android) guide](/platforms/termux) — repurpose an old phone as a gateway
