...
Hex features that can generate provider usage or paid API calls.
...
- `hex status --usage` and `hex channels list` show provider **usage windows**
...
Hex can pick up credentials from:
...
`hex models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
