...
Hex integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Hex owns lifecycle when `channels.signal.autoStart=true`.
...
- Hex spawns `imsg rpc` as a child process (legacy iMessage integration).
