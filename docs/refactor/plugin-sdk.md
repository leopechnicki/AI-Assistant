...
- Publish as `hex/plugin-sdk` (or export from core under `hex/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `HexPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: HexConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: HexConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: HexConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: HexConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: HexConfig): string;
  };
};
```
...
- Introduce `hex/plugin-sdk`.
- Add `api.runtime` to `HexPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `hexRuntime: ">=2026.2.0"`).
