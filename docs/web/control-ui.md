...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/hex`)
...
If the page fails to load, start the Gateway first: `hex gateway`.
...
you revoke it with `hex devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.hex/hex.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Hex
...
Hex **blocks** Control UI connections without device identity.
