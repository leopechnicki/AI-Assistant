...
- Jobs persist under `~/.hex/cron/` so restarts don’t lose schedules.
...
hex cron list
hex cron run <job-id> --force
hex cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.hex/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `hex cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Hex defaults to `announce`.
...
- Job store: `~/.hex/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.hex/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
hex cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
hex cron edit <jobId> --agent ops
hex cron edit <jobId> --clear-agent
```
...
```bash
hex system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`hex system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `HEX_SKIP_CRON`.
