...
summary: 设置指南：在保持最新的同时保持你的 Hex 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.hex/workspace`（工作区）+ `~/.hex/hex.json`（配置）。
...
- **配置：** `~/.hex/hex.json`（JSON/JSON5 格式）
- **工作区：** `~/.hex/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm hex setup` 运行它。
...
- 运行 `hex setup`，然后 `hex channels login`，然后手动启动 Gateway 网关（`hex gateway`）。
...
- **配对允许列表**：`~/.hex/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.hex/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.hex/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.hex/workspace` 和 `~/.hex/` 保持为"你的东西"；不要将个人提示/配置放入 `hex` 仓库。
