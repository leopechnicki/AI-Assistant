...
所有日志配置都在 `~/.hex/hex.json` 的 `logging` 下。
...
- Hex 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `hex plugins enable diagnostics-otel` 启用插件。
...
- `hex.tokens`（计数器，属性：`hex.token`、`hex.channel`、`hex.provider`、`hex.model`）
- `hex.cost.usd`（计数器，属性：`hex.channel`、`hex.provider`、`hex.model`）
- `hex.run.duration_ms`（直方图，属性：`hex.channel`、`hex.provider`、`hex.model`）
- `hex.context.tokens`（直方图，属性：`hex.context`、`hex.channel`、`hex.provider`、`hex.model`）

消息流：

- `hex.webhook.received`（计数器，属性：`hex.channel`、`hex.webhook`）
- `hex.webhook.error`（计数器，属性：`hex.channel`、`hex.webhook`）
- `hex.webhook.duration_ms`（直方图，属性：`hex.channel`、`hex.webhook`）
- `hex.message.queued`（计数器，属性：`hex.channel`、`hex.source`）
- `hex.message.processed`（计数器，属性：`hex.channel`、`hex.outcome`）
- `hex.message.duration_ms`（直方图，属性：`hex.channel`、`hex.outcome`）

队列 + 会话：

- `hex.queue.lane.enqueue`（计数器，属性：`hex.lane`）
- `hex.queue.lane.dequeue`（计数器，属性：`hex.lane`）
- `hex.queue.depth`（直方图，属性：`hex.lane` 或 `hex.channel=heartbeat`）
- `hex.queue.wait_ms`（直方图，属性：`hex.lane`）
- `hex.session.state`（计数器，属性：`hex.state`、`hex.reason`）
- `hex.session.stuck`（计数器，属性：`hex.state`）
- `hex.session.stuck_age_ms`（直方图，属性：`hex.state`）
- `hex.run.attempt`（计数器，属性：`hex.attempt`）
...
- `hex.webhook.error`
  - `hex.channel`、`hex.webhook`、`hex.chatId`、`hex.error`
- `hex.message.processed`
  - `hex.channel`、`hex.outcome`、`hex.chatId`、`hex.messageId`、`hex.sessionKey`、`hex.sessionId`、`hex.reason`
- `hex.session.stuck`
  - `hex.state`、`hex.ageMs`、`hex.queueDepth`、`hex.sessionKey`、`hex.sessionId`
