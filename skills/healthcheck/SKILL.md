---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Hex deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Hex cron scheduling for periodic checks, or version status checks on a machine running Hex (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Hex, then align it to a user-defined risk tolerance without breaking access. Use Hex security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Hex changes the host firewall, SSH, or OS updates; it does not.
...
5. Hex gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Hex can access sensitive data.
...
### 2) Run Hex security audits (read-only)

As part of the default read-only checks, run `hex security audit --deep`. Only offer alternatives if the user requests them:

1. `hex security audit` (faster, non-probing)
2. `hex security audit --json` (structured output)

Offer to apply Hex safe defaults (numbered):

1. `hex security audit --fix`

Be explicit that `--fix` only tightens Hex defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Hex version/update status (read-only)

As part of the default read-only checks, run `hex update status`.
...
- Credential hygiene notes (location of Hex creds, prefer disk encryption)
...
After Hex install or first hardening pass, run at least one baseline audit and version check:

- `hex security audit`
- `hex security audit --deep`
- `hex update status`

Ongoing monitoring is recommended. Use the Hex cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `hex cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `hex update status`
...
Before creating, `hex cron list` and match on exact `name`. If found, `hex cron edit <id> ...`.
If not found, `hex cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `hex update status` (preferred for source checkouts and channels)
2. `npm view hex version` (published npm version)

## Hex command accuracy

Use only supported commands and flags:

- `hex security audit [--deep] [--fix] [--json]`
- `hex status` / `hex status --deep`
- `hex health --json`
- `hex update status`
- `hex cron add|list|runs|run`

Do not invent CLI flags or imply Hex enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Hex compaction:
