...
This document describes how the Hex ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`hex acp` exposes an ACP agent over stdio and forwards prompts to a running
Hex Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Hex Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `hex acp` over stdio.

Example config:

```bash
hex config set gateway.remote.url wss://gateway-host:18789
hex config set gateway.remote.token <token>
```

Example run:

```bash
hex acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
hex acp --session agent:main:main
hex acp --session agent:design:main
hex acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Hex ACP” to start a thread.

## Execution Model

- ACP client spawns `hex acp` and speaks ACP messages over stdio.
...
```bash
hex acp --session agent:main:main
hex acp --session-label "support inbox"
hex acp --reset-session
```
...
`hex acp` resolves the Gateway URL and auth from CLI flags or config:
