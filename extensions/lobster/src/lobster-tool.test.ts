...
import type { HexPluginApi, HexPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "hex-lobster-plugin-") {
...
function fakeApi(overrides: Partial<HexPluginApi> = {}): HexPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<HexPluginToolContext> = {}): HexPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: HexPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
