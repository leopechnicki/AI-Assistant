...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as HexConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as HexConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as HexConfig);
