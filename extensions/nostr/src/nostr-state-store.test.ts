import type { PluginRuntime } from "hex/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.HEX_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-nostr-"));
  process.env.HEX_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.HEX_STATE_DIR?.trim() || env.HEX_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".hex");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.HEX_STATE_DIR = previous;
    }
