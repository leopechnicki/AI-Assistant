...
metadata: { "hex": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Hex `sessions_spawn`
- **File I/O** == Hex `read`/`write`
- **Remote fetch** == Hex `web_fetch` (or `exec` with curl when POST is required)
