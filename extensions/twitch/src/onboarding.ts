...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: HEX_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: HexConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var HEX_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.HEX_TWITCH_ACCESS_TOKEN?.trim();
