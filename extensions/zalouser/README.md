...
Hex extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
hex channels login --channel zalouser
...
```bash
hex message send --channel zalouser --target <threadId> --message "Hello from Hex!"
```
...
```bash
hex channels login --channel zalouser              # Login via QR
hex channels login --channel zalouser --account work
hex channels status --probe
hex channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
hex directory self --channel zalouser
hex directory peers list --channel zalouser --query "name"
hex directory groups list --channel zalouser --query "work"
hex directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
hex message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
hex message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `hex channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
hex channels login --channel zalouser --account work
hex message send --channel zalouser --account work --target <id> --message "Hello"
