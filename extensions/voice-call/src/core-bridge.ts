...
function resolveHexRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.HEX_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set HEX_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveHexRoot(), "dist", "extensionAPI.js");
