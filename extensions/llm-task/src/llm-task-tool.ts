...
// NOTE: This extension is intended to be bundled with Hex.
// When running from source (tests/dev), Hex internals live under src/.
...
export function createLlmTaskTool(api: HexPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via hex.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "hex-llm-task-"));
