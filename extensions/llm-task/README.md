...
`hex.invoke --each`) without adding new Hex code per workflow.
...
This extension depends on Hex internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Hex extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.hex/extensions` as a standalone plugin directory.
