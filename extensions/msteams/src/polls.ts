...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["hexPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["hex", "pollId"]) ??
    readNestedString(value, ["hex", "poll", "id"]) ??
    readNestedString(value, ["data", "hexPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "hex", "pollId"]);
  if (!pollId) {
    return null;
  }
