import type { HexApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as HexApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as HexApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as HexApp);
  }
  if (host.tab === "agents") {
    const app = host as unknown as HexApp;
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as HexApp);
    await loadDevices(host as unknown as HexApp);
    await loadConfig(host as unknown as HexApp);
    await loadExecApprovals(host as unknown as HexApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as HexApp);
    await loadConfig(host as unknown as HexApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as HexApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as HexApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__HEX_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as HexApp, false),
    loadPresence(host as unknown as HexApp),
    loadSessions(host as unknown as HexApp),
    loadCronStatus(host as unknown as HexApp),
    loadDebug(host as unknown as HexApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as HexApp, true),
    loadConfigSchema(host as unknown as HexApp),
    loadConfig(host as unknown as HexApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as HexApp, false),
    loadCronStatus(host as unknown as HexApp),
    loadCronJobs(host as unknown as HexApp),
  ]);
}
