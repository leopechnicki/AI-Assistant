...
const originalConnect = HexApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("openclaw-app") as HexApp;
...
beforeEach(() => {
  HexApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__HEX_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  HexApp.prototype.connect = originalConnect;
  window.__HEX_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("honors explicit base path overrides", async () => {
    window.__HEX_CONTROL_UI_BASE_PATH__ = "/openclaw";
