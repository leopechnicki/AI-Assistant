          ...
          state.tab === "overview"
            ? renderOverview({
                ...
                onSessionKeyChange: (next) => {
                  ...
                  state.chatMessage = "";
                  (state as unknown as HexApp).resetToolStream();
                  state.applySettings({
                    ...state.settings,
                    sessionKey: next,
          ...
          state.tab === "chat"
            ? renderChat({
                ...
                onSessionKeyChange: (next) => {
                  ...
                  (state as unknown as HexApp).chatStreamStartedAt = null;
                  state.chatQueue = [];
                  (state as unknown as HexApp).resetToolStream();
                  (state as unknown as HexApp).resetChatScroll();
                  state.applySettings({
                    ...state.settings,
                    sessionKey: next,
                    lastActiveSessionKey: next,
                ...
                onChatScroll: (event) => (state as unknown as HexApp).handleChatScroll(event),
                onDraftChange: (next) => (state.chatMessage = next),
                attachments: state.chatAttachments,
                onAttachmentsChange: (next) => (state.chatAttachments = next),
                onSend: () => (state as unknown as HexApp).handleSendChat(),
                canAbort: Boolean(state.chatRunId),
                onAbort: () => void (state as unknown as HexApp).handleAbortChat(),
                onQueueRemove: (id) => (state as unknown as HexApp).removeQueuedMessage(id),
                onNewSession: () =>
                  (state as unknown as HexApp).handleSendChat("/new", { restoreDraft: true }),
                showNewMessages: state.chatNewMessagesBelow,
                onScrollToBottom: () => state.scrollToBottom(),
                // Sidebar props for tool output viewing
                sidebarOpen: (state as unknown as HexApp).sidebarOpen,
                sidebarContent: (state as unknown as HexApp).sidebarContent,
                sidebarError: (state as unknown as HexApp).sidebarError,
                splitRatio: (state as unknown as HexApp).splitRatio,
                onOpenSidebar: (content: string) =>
                  (state as unknown as HexApp).handleOpenSidebar(content),
                onCloseSidebar: () => (state as unknown as HexApp).handleCloseSidebar(),
                onSplitRatioChange: (ratio: number) =>
                  (state as unknown as HexApp).handleSplitRatioChange(ratio),
        ...
        ${
          state.tab === "config"
            ? renderConfig({
                ...
                searchQuery: (state as unknown as HexApp).configSearchQuery,
                activeSection: (state as unknown as HexApp).configActiveSection,
                activeSubsection: (state as unknown as HexApp).configActiveSubsection,
                onRawChange: (next) => {
                  state.configRaw = next;
                },
                onFormModeChange: (mode) => (state.configFormMode = mode),
                onFormPatch: (path, value) =>
                  updateConfigFormValue(state as unknown as HexApp, path, value),
                onSearchChange: (query) =>
                  ((state as unknown as HexApp).configSearchQuery = query),
                onSectionChange: (section) => {
                  (state as unknown as HexApp).configActiveSection = section;
                  (state as unknown as HexApp).configActiveSubsection = null;
                },
                onSubsectionChange: (section) =>
                  ((state as unknown as HexApp).configActiveSubsection = section),
                onReload: () => loadConfig(state as unknown as HexApp),
                onSave: () => saveConfig(state as unknown as HexApp),
                onApply: () => applyConfig(state as unknown as HexApp),
                onUpdate: () => runUpdate(state as unknown as HexApp),
              })
            : nothing
        }
          ...
          state.tab === "logs"
            ? renderLogs({
                ...
                onExport: (lines, label) =>
                  (state as unknown as HexApp).exportLogs(lines, label),
                onScroll: (event) => (state as unknown as HexApp).handleLogsScroll(event),
              })
            : nothing
        }
      </main>
